...
class SusiBO:
    def __init__(self, test, init_points:int, n_iter: int):
        ...
        f=self.loss, pbounds=self.bounds,
        random_state=42,  
    ...
    def plot_loss_(self):
        ...
        y = [self.loss(a=2.0, nu=0.72, ph=val) for val in a]

        # bisherige Versuche einzeichnen
        a_tries = [res['params']['nu'] for res in self.optimizer.res]
        y_tries = [self.loss(a=2.0, nu=0.72, ph=val) for val in a_tries]

        plt.plot(a, y, label="Loss Function")
        plt.scatter(a_tries, y_tries, color="red", zorder=5, label="Tries")
        plt.show()
